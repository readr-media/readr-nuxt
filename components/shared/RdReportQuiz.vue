...
export default {
  ...
  props: {
    ...
    quizOptions: {
      ...
      validator(value) {
        return (
          value.length <= 4 &&
          value.reduce(function checkKeyOfTypeAndTextExistInEveryOption(
            acc,
            curr
          ) {
            return (
              acc &&
              Object.prototype.hasOwnProperty.call(curr, 'type') &&
              Object.prototype.hasOwnProperty.call(curr, 'text')
            )
          },
          true)
        )
  ...
  methods: {
    handleQuizButtonClick(option, i) {
      ...
      this.$emit('quizButtonClick', { option, index: i })
